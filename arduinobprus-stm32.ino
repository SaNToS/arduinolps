...
#include <EEPROM.h>
#include "ACS712.h"
//ACS712 sensor(ACS712_20A, PA0);

LiquidCrystal lcd(PB11, PB6, PB5, PB4, PB3, PB14); //rs, e, d4, d5, d6, d7
...
const float maxI = 8.00;  //максимальный ток
int button1 = PB13;          //кнопка 1
int button2 = PB12;          //кнопка 2
float ah = 0.0000;        //Cчетчик Ампер*часов
const int down = PB10;      //выход валкодера 1/2
const int up =  PB8;        //выход валкодера 2/2
const int pwm = PA9;        //выход ШИМ 
const int power = PB7;      //управление релюхой
...
{  
  ...
  if (Serial.available() > 0) {  //если есть доступные данные
    ...
    if(incomingByte==105){
      ...
      delay(100);
      digitalWrite(PA6, 0); //гасим красный светодиод 
      Serial.print("t0;");
      off = false;
      set = 0;//выходим из меню
   }  
  ...
  float Uout = analogRead(PA1) * ((5.0 + Ucorr) / 4095.0) * 5.0; //узнаем напряжение на выходе
//float Iout = 1;
  float Iout = analogRead(PA0) / 100.00; // узнаем ток в нагрузке
  //float Iout = sensor.getCurrentAC();
...
{   
  ...
  { 
    digitalWrite(power, 0); //вырубаем реле
    Serial.print("t2;");    
    digitalWrite(PA6, 1);   //зажигаем красный светодиод
    level = 0; //убираем ШИМ сигнал
...
if(level>65535) level = 65535; //не поднимаем ШИМ выше 13 бит
//Все проверили, прощитали и собственно отдаем команду для силового транзистора.
if(ceil(level)!=255) pwmWrite(pwm, ceil(level)); //подаем нужный сигнал на ШИМ выход (кроме 255, так как там какая-то лажа!!!)
...
if(currentMillis - com2 > com) {
    ...
    Serial.print('r');    Serial.print(digitalRead(PA6));    Serial.print(';');    
    Serial.print('b');    Serial.print(digitalRead(PA5));    Serial.print(';');    
    Serial.println(' ');  
  }  
